...
public class SharpIrCompiler : ICompiler
{
        ...
        private void GenerateObjectFileContent(string sourceFilePath, string oirOutputPath, string header)
    {
        ...
        compiler.StartInfo.FileName = "sharpir";
        	...
        	compiler.StartInfo.Arguments = $"\"{sourceFilePath}\"" + extraArgs + " --emit-json --out \"{oirOutputPath}\"";
    ...
    [StarterTemplate("sharpir")]
    public static (string MainFile, string Content) GetStarter(string name)
    {
        var mainFile = "src/main.sir";
        var mainContent = $@"// {name} - SharpIR main
    // Generated by Meow
